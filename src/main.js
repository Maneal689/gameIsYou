...
function Entity(id, x_pos, y_pos, text = null) {
    ...
    this.isBlock = function() {
        if (
            this.property == P_STOP ||
            this.property == P_PUSH ||
            this.id == REAL_WALL
        )
            return true;
        return false;
    ...
    this.changeProperties = function(n_id) {
        if (n_id >= 200) this.id = n_id - 200;
        else if (n_id >= 100) this.property = n_id;
    };
}
...
function Level(name = null) {
    ...
    this.findIs = function() {
        ...
        for (i = 0; i < this.items.length; i++) {
            ...
            if (current.id == N_IS) res.push(current);
        }
        return res;
    };
    ...
    this.changeProperties = function(idChanging, nProp) {
        ...
        if (nProp != 200) {
            for (i = 0; i < this.items.length; i++) {
                current = this.items[i];
                if (current.id == idChanging) current.changeProperties(nProp);
            }
        }
    };
    ...
    this.moveBlock = function(x, y, item) {
        if (!item.isBlock()) return true;
            ...
            if (
                this.moveBlock(x, y, this.getOn(item.x_pos + x, item.y_pos + y))
            ) {
                item.x_pos += x;
                item.y_pos += y;
                return true;
            ...
            if (current.property == P_YOU) {
                if (
                    this.moveBlock(
                        x,
                        y,
                        this.getOn(current.x_pos + x, current.y_pos + y)
                    )
                ) {
                    current.x_pos += x;
                    current.y_pos += y;
    ...
    this.getPlayerMinPos = function() {
        ...
        for (i = 0; i < this.items.length; i++) {
            if (
                this.items[i].property == P_YOU &&
                (this.items[i].x_pos < res['x'] || res['x'] == null)
            )
                res['x'] = this.items[i].x_pos;
            if (
                this.items[i].property == P_YOU &&
                (this.items[i].y_pos < res['y'] || res['y'] == null)
            )
    ...
    this.pWinList = function() {
        ...
        for (i = 0; i < this.items.length; i++) {
            if (this.items[i].property == P_WIN) res.push(this.items[i]);
        }
        return res;
    };
    ...
    this.pYouList = function() {
        ...
        for (i = 0; i < this.items.length; i++) {
            if (this.items[i].property == P_YOU) res.push(this.items[i]);
        }
        return res;
    };
    ...
    this.isWin = function() {
        ...
        for (i = 0; i < pYou.length; i++) {
            for (j = 0; j < pWin.length; j++) {
                if (
                    pYou[i].x_pos == pWin[j].x_pos &&
                    pYou[i].y_pos == pWin[j].y_pos
                )
                    return true;
            }
        }
...
function parseJsonLevels(jsonLvl) {
    ...
    for (i = 0; i < keys.length; i++) {
        ...
        for (j = 0; j < currentLvl.items.length; j++) {
            res[i].addEntity(
                dicoID[currentLvl.items[j].id],
                currentLvl.items[j].x_pos,
                currentLvl.items[j].y_pos
            );
...
function getSelectedLvl(menu) {
    ...
    for (i = 0; i < menu.items.length; i++) {
        if (
            menu.items[i].x_pos == xPos &&
            menu.items[i].y_pos == yPos &&
            menu.items[i].id == CHECKBOX
        ) {
...
function startMenu() {
    ...
    docInfo['previewMaxHeight'] = docInfo['previewCanva'].getAttribute(
        'height'
    );
    ...
    drawLevel(
        menu,
        docInfo['gameCtx'],
        docInfo['gameMaxWidth'],
        docInfo['gameMaxHeight'],
        true
    );
        ...
        if (change) {
            ...
            if (selected != null)
                drawLevel(
                    levels[selected],
                    docInfo['previewCtx'],
                    docInfo['previewMaxWidth'],
                    docInfo['previewMaxHeight'],
                    false
                );
            drawLevel(
                menu,
...
function startGame(level, ctx, maxWidth, maxHeight) {
    ...
    document.body.onkeypress = function(e) {
        ...
        if (e.key == 'ArrowUp') change = level.movePlayer(0, -1, true);
        else if (e.key == 'ArrowDown') change = level.movePlayer(0, 1, true);
        else if (e.key == 'ArrowRight') change = level.movePlayer(1, 0, true);
        else if (e.key == 'ArrowLeft') change = level.movePlayer(-1, 0, true);
        else if (e.key == 'r') {
            level.resetLevel();
            change = true;
...
function drawLevel(level, ctx, maxWidth, maxHeight, view) {
    ...
    ctx.drawImage(
        ...
        0,
        0,
        GameImg['background'].width,
        GameImg['background'].height,
        0,
        0,
        maxWidth,
        maxHeight
        ...
        if (level.items[i].id != TEXT) {
            ...
            if (!view || middlePos == null) {
                ...
                ctx.drawImage(
                    ...
                    currentImg.width,
                    currentImg.height,
                    c_x * x_size + maxWidth / 2,
                    c_y * y_size + maxHeight / 2,
                    x_size,
                    y_size
                );
            }
